...
def save_to_excel(soup):
    global n
    try:
        title = soup.find(class_='ok-post-header').get_text()
        summary = ""
        published_time = soup.find(class_='ok-post-date').get_text()
        texts = soup.find(class_='post-content-wrap tbl-forkorts-article tbl-forkorts-article-active') or \
            soup.find(class_='post-content-wrap')
...
if __name__ == '__main__':
    ...
    while page <= 31:
        url = f'https://english.onlinekhabar.com/category/political/page/{page}'
       ...
       # get the list
        ...
        try:
            items = soup.find(
                class_='listical-news-big').find_all(class_='ok-news-post ltr-post')
        except Exception as e:
